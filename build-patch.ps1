...
function Build-PatchFromAsm {
    ...
    $tempDir = Join-Path $env:TEMP "mcpatcher_build_$(Get-Random)"
    ...
    try {
        ...
        $process = Start-Process -FilePath $ML64Path -ArgumentList $assembleArgs -NoNewWindow -Wait -PassThru -RedirectStandardError (Join-Path $tempDir "error.txt")
        
        if ($process.ExitCode -ne 0) {
            $errorContent = Get-Content (Join-Path $tempDir "error.txt") -Raw
...
try {
    ...
    if ($All) {
        ...
        $asmFiles = Get-ChildItem -Path . -Filter "*.asm" | Where-Object { -not $_.Name.StartsWith("_") }
    } elseif ($AsmFile) {
        ...
        # Default: process all .asm files in current directory
        $asmFiles = Get-ChildItem -Path . -Filter "*.asm" | Where-Object { -not $_.Name.StartsWith("_") }
    }
    ...
    foreach ($asmFile in $asmFiles) {
        if ($Verify) {
            Verify-Patch -AsmFilePath $asmFile.FullName
        } else {
            if (Build-PatchFromAsm -AsmFilePath $asmFile.FullName -ML64Path $ml64Path) {
                $successCount++
            } else {
                $failCount++
            }
